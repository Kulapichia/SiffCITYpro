...
export async function GET(request: NextRequest) {
  ...
  if (type === 'internal') {
    ...
    try {
      ...
      const formattedDanmu = (danmuList || []).map((item: any) => ({
        text: item.text,
        color: item.color,
        mode: item.mode,
        time: item.time,
      ...
      console.error('获取内部弹幕失败:', error);
      return NextResponse.json({ error: '获取内部弹幕失败' }, { status: 500 });
    }
  }
