...
export function TelegramAuthConfig({ config, onSave }: TelegramAuthConfigProps) {
  ...
  return (
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-800/50 rounded-lg'>
        <div>
          <label htmlFor='defaultRole' className='text-sm font-medium text-gray-900 dark:text-gray-100'>
            新用户默认角色
          </label>
          <p className='text-xs text-gray-500 dark:text-gray-400 mt-1'>
            通过 Telegram 自动注册的新用户将被分配此角色
          </p>
        </div>
        <select
          id='defaultRole'
          value={localConfig.defaultRole || 'user'}
          onChange={(e) => setLocalConfig({ ...localConfig, defaultRole: e.target.value as 'user' | 'admin' })}
